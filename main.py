...
#topbar { background:#f7f7f5 !important; border-bottom:1px solid #e5e5e0 !important; padding:4px 8px !important; }
#topbar button { min-width:36px !important; height:32px !important; }
#sidebar { background:#fff !important; border-right:1px solid #e5e5e0 !important; }
#sidebar button { width:100% !important; margin-bottom:4px !important; }
"""
...
with gr.Blocks(theme=gr.themes.Base(), title="pig", css=CSS) as demo:
    ...
    with gr.Column(visible=False) as chat_page:
        ...
        with gr.Row(elem_id="topbar"):
            ...
            gr.HTML("<div style='flex:1;text-align:center;font-size:15px;font-weight:600;line-height:32px;'>📐 pig</div>")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, visible=False, elem_id="sidebar") as sidebar:
                gr.HTML("<div style='padding:10px 12px;font-weight:600;border-bottom:1px solid #e5e5e0;margin-bottom:8px;'>📋 历史提问</div>")
                del_btn    = gr.Button("🗑 删除最近一条", size="sm")
                clear_btn  = gr.Button("⚠️ 清空全部",    size="sm")
                close_btn  = gr.Button("✕ 关闭",         size="sm")
                hist_status = gr.HTML("")
                hist_box   = gr.Chatbot(show_label=False, height=480)
    ...
    logout_btn.click(handle_logout, [], [auth_page, chat_page, logged_in_user, logged_in_nick, chatbot, sidebar])
    clr_btn.click(lambda: [], [], [chatbot])
    menu_btn.click(open_sidebar, [logged_in_user], [sidebar, hist_box, hist_status])
    close_btn.click(close_sidebar, [], [sidebar])
    del_btn.click(do_del,   [logged_in_user], [hist_box, hist_status, chatbot])
    clear_btn.click(do_clear, [logged_in_user], [hist_box, hist_status, chatbot])
